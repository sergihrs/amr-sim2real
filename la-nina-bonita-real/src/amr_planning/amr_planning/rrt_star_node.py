...
class AStarNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Initializing A* node...")
    ...
    def _path_callback(self, pose_msg: AmrPoseStamped):
        ...
        if pose_msg.localized and not self._localized:
            ...
            self.get_logger().info(f"Executing A* from {start} to {self._goal}...")
            path = self._planning.compute_path(start, self._goal)
            self.get_logger().info(f"A* found path with {len(path)} nodes.")
            ...
            # )
            smoothed_path = path
            self.get_logger().info(f"Smoothed path has {len(smoothed_path)} nodes.")
