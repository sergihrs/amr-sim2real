...
class ParticleFilter:
    ...
    def __init__(
        ...
        self._map = Map(
            map_path, sensor_range, compiled_intersect=True, use_regions=True
        )
    ...
    def resample(self, measurements: list[float]) -> None:
        ...
        if self._likelihood is None:
            self._likelihood = weights.mean()
        else:
            self._likelihood = (
                self._beta_ema * weights.mean()
                + (1 - self._beta_ema) * self._likelihood
            )
        self._logger.info(
            f"Mean weight: {weights.mean():.4f} | likelihood: {self._likelihood:.4f}"
        )

        if self._likelihood < 0.005 and self._iteration > 3:
            self._logger.info("Too low mean weight. Resampling")
    ...
    def _measurement_probability(
        ...
        particle_measurements = np.fromiter(
            particle_measurements, dtype=float, count=N_LIDAR_RAYS
        )
        filtered_measurements = np.fromiter(
            filtered_measurements, dtype=float, count=N_LIDAR_RAYS
        )

        nans = np.isnan(filtered_measurements) | np.isnan(particle_measurements)

        filtered_measurements = filtered_measurements[~nans]
        particle_measurements = particle_measurements[~nans]
