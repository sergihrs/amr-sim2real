...
class ParticleFilterNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("steps_btw_sense_updates", 3)
        ...
        self.get_logger().info(
            f"Initialazing particle filter with {particles} particles."
        )
        self._steps_btw_sense_updates_localized = 2
    ...
    def _compute_pose_callback(self, odom_msg: Odometry, scan_msg: LaserScan):
        ...
        if self._prev_pose and self._last_pose_time:
            ...
            # 
            theta_x, theta_y = np.cos(theta), np.sin(theta)
            p_theta_x, p_theta_y = np.cos(p_theta), np.sin(p_theta)
            z_w = np.arctan2(
                theta_y * p_theta_x - theta_x * p_theta_y,
                theta_x * p_theta_x + theta_y * p_theta_y,
            ) / dt
        ...
        # self.get_logger().info(f"ODOMETRIA -> z_v: {z_v}, z_w: {z_w}")
    ...
    def _execute_measurement_step(
        ...
        sbsu = self._steps_btw_sense_updates_localized if self._localized else self._steps_btw_sense_updates

        if self._steps % sbsu == 0:
            ...
            end_stop_time = time.perf_counter() - start_stop_time

            self._particle_filter.substract_stop_time(end_stop_time)

            return pose

        return pose if not self._localized else self._previous_pose
