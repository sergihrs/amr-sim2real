...
#     """
#     filtered_ranges = [d if (not math.isnan(d)) else 1000 for d in z_scan]
#     target_index = int(target_angle * len(filtered_ranges) / 360)

#     min_distance, argmin_angle = 8, 4
#     for i in range(-width // 2, width // 2 + 1):
#         index = (target_index + i) % len(filtered_ranges)
#         if filtered_ranges[index] < min_distance:
#             min_distance = filtered_ranges[index]
#             argmin_angle = index * 2 * math.pi / len(filtered_ranges)

#     return min_distance, argmin_angle


def raycast(z_scan: list[float], target_angle: float, width: int) -> tuple[float, float]:
    """
    Raycast function to detect obstacles in a given direction.
    Parameters
    ----------
    ...
    float: Angle to the closest obstacle [rad].
    """
    # Get the indexes and distances of the rays to consider
    distances, indexes = get_rays(z_scan, target_angle, width)
...
def get_rays(z_scan: list[float], target_angle: float, width: int) -> tuple[list[float], list[int]]:
    """
    ...
    indexes = np.arange(target_index - width // 2, target_index + width // 2 + 1) % len(z_scan)
...
class InitialState(RobotState):
    def __init__(self) -> None:
        super().__init__()
        self._found_wall = False

    def exit_state(self, z_scan: list[float]) -> None:
        if not self._found_wall and all(ray < 0.4 for ray in get_rays(z_scan, 270, 30)[0]):
            self._found_wall = True

        if (
            self._found_wall
            and 3 * math.pi / 2 - 0.1 < raycast(z_scan, 270, 69)[1] < 3 * math.pi / 2 + 0.1
        ):
            return FollowWall()

    @RobotState.execute_decorator
    def execute(self, z_scan: list[float], z_v: float, z_w: float) -> tuple[float, float]:
...
class FollowWall(RobotState):
    ...
    def exit_state(self, z_scan: list[float]) -> None:
        ...
        if raycast(z_scan, 270, 3)[0] > 2 * DESIRED_DISTANCE:
            return InnerTurn()
    ...
    def execute(self, z_scan: list[float], z_v: float, z_w: float) -> tuple[float, float]:
        ...
        kp1, kd1, kp2 = 10, 1, 1
        w_pd = (
            kp1 * distance_error + kd1 * (distance_error - self._previous_error) / self._delta_time
        )
...
class OuterTurn(RobotState):
    ...
    def execute(self, z_scan: list[float], z_v: float, z_w: float) -> tuple[float, float]:
        ...
        self._current_angle += z_w * self._delta_time
...
class InnerTurn(RobotState):
    ...
    def exit_state(self, z_scan: list[float]) -> None:
        if self._step == 2 and raycast(z_scan, 270, 3)[0] < DESIRED_DISTANCE + DESIRED_THRESHOLD:
            return FollowWall()
    ...
    def execute(self, z_scan: list[float], z_v: float, z_w: float) -> tuple[float, float]:
        ...
        """
        if self._step == 0:
            self._current_distance += z_v * self._delta_time
